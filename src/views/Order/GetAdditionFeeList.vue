<template>
  <div class="feelist">附加费
    <div>
      <div class="top">
        ...
        <div class="block">
          ...
          <el-select v-model="value3" placeholder="状态">
            <el-option v-for="item in orderService" :key="item.value" :label="item.titie" :value="item.val">
            </el-option>
          </el-select>
          <!-- 时间 -->
      ...
      <!-- 附加费列表 -->
      <div>
        <el-table :data="rightsDate" border stripe @row-click="fromDetailsdblClick">
        ...
        <!-- 分页 -->
        <div class="block">
          <el-pagination @current-change="handleCurrentChange" :current-page.sync="currentPage3" :page-size=setData.pageSize layout="prev, pager, next, jumper" :total=setData.total>
          </el-pagination>
        </div>
	...
	methods: {
		...
		fromDetailsdblClick(e) {
			...
			this.$router.push({
				...
				query: {orderAdditionalFeeID: e.orderAdditionalFeeID}
			});
		}
	}
};
